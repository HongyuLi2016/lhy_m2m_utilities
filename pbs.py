...
if __name__ == '__main__':
  ...
  for ncfg in cfg_files:
    ...
    with open('%s/pbsscript/%s.pbs'%(options.gname,model_name),'w') as ff:
      ...
      print >>ff, '#PBS -o %s/%s/rst_%s/'%(path,options.gname,model_name)
      print >>ff, '#PBS -e %s/%s/rst_%s/'%(path,options.gname,model_name)
      print >>ff, '#PBS -l nodes=1:ppn=32'
      print >>ff, 'nproc=`cat $PBS_NODEFILE | wc -l`'
      print >>ff, 'cat $PBS_NODEFILE > NODEFILE'
      print >>ff, 'cd %s'%(path)
      print >>ff, '/home/lhy/bin/prepare.sh %s %s'%(options.gname,model_name)
